# 👋 Hi, I'm Bader Aljaberi

**Computer Science Student | Python Developer | AI Enthusiast**

Passionate about building practical software solutions and exploring advanced computer science concepts. Currently focused on AI applications, automation, and compiler technologies.

## 🚀 Featured Projects

### [SmartDocs](https://github.com/Bader-Aljaberi/SmartDocs)
Python documentation generator using Abstract Syntax Trees (AST) for automated Google-style docstring generation.

### [NASA BioScience Engine](https://github.com/Bader-Aljaberi/NASA-BioScience-Knowledge-Engine)
Knowledge engine for NASA bioscience data with advanced search and analysis features.

### [Nasser Bot](https://github.com/Bader-Aljaberi/Nasser-Bot)  
GCC countries information chatbot with natural language processing capabilities.

### [Voice Bridge](https://github.com/Bader-Aljaberi/Voice-Bridge)
Real-time voice translation system demonstrating AI integration and multimedia processing.

## 💻 Technical Skills

**Languages:** Python, JavaScript, HTML/CSS, Java  
**Technologies:** Git, Jupyter, AST Parsing, APIs, Data Analysis  
**Concepts:** Data Structures, Algorithms, OOP, Software Architecture

## 📊 GitHub Stats

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=Bader-Aljaberi&show_icons=true&theme=default)

## 📫 Let's Connect

- [LinkedIn](https://www.linkedin.com/in/bader-aljaberi-a593b8367/)
- [Email](badran.aljaberi@gmail.com) 
---

*"Building solutions one line at a time"*
